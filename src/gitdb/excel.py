...
def _is_control_character(character: str) -> bool:
    ...
    # in element text). DEL and the C1 range are control characters too, but
    # a plain ``< " "`` check would miss them.
    return character < " " or character == "\x7f" or "\x80" <= character <= "\x9f"


def _sheet_name(name: str, used: Sequence[str]) -> str:
    cleaned = "".join(
        " " if character in _ILLEGAL_SHEET_CHARS or _is_control_character(character) else character
